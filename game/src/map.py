...
from config import WIN_WIDTH, WIN_HEIGHT, YELLOW, MAP_TILE_SIZE, MAP_BORDER_SIZE, MAP_IMAGE_PATHS
import pygame
...
def combine_images(matrix):
    ...
    for y, row in enumerate(matrix):
        for x, value in enumerate(row):
            ...
            if img:
                combined_surface.blit(img, (x * MAP_TILE_SIZE + MAP_BORDER_SIZE, y * MAP_TILE_SIZE + MAP_BORDER_SIZE))

    border_color = (81, 81, 81)
    pygame.draw.rect(combined_surface, border_color, combined_surface.get_rect(), MAP_BORDER_SIZE)
